...
//-----------------------------------------------------------------------------
static int32_t blk_up_set(app_tuds_t *p_app_UD, uint8_t *pkt, uint16_t len)
{
//    uint8_t  j;
    uint16_t i;
    uint16_t cs;
...
//-----------------------------------------------------------------------------
static int32_t blk_up_get_Ucmd(app_tuds_t *p_app_UD, uint8_t *buf20 )
{
    uint8_t  i;
...
//-----------------------------------------------------------------------------
static int32_t blk_up_get_blk_No(app_tuds_t *p_app_UD, uint8_t *buf20, uint8_t blk_No )
{
    uint16_t position;
    uint8_t  i;
...
static int32_t blk_up_setSent(app_tuds_t *p_app_UD, uint8_t blk_No )
{
    uint16_t position;
    position = blk_No;
    if( ( (position * 16) + 1) > (p_app_UD->m_blkUp_len))
...
static int32_t blk_up_getNextBlkno(app_tuds_t *p_app_UD, uint8_t current_blk_No)
{
    int32_t r;
    uint16_t i;
...
//
//-----------------------------------------------------------------------------
PUBLIC uint32_t app_tuds_BlkUp_timer_init() //uint32_t ticks_per_100ms)
{
    uint32_t  err_code;
...
//
//-----------------------------------------------------------------------------
static uint32_t  BlkUp_timer_start(app_tuds_t *p_app_UD, uint32_t timeout_ticks)
{
    uint32_t  err_code;
    //timeout_ticks = 1000;
...
//SAD
    ...
    err_code = app_timer_start(m_BlkUp_timer_id, timeout_ticks, p_app_UD);
    if( err_code != NRF_SUCCESS )
    {
...
//
//-----------------------------------------------------------------------------
static uint32_t blk_up_startSend_Ucmd(app_tuds_t *p_app_UD)
{
    uint32_t err_code;
    ...
    m_Ucmd_len = 20;
    err_code = ble_tuds_notify_Ucmd(p_app_UD->p_ble_tuds, m_Ucmd_buf, &m_Ucmd_len);
    if(err_code == NRF_SUCCESS)
    {
        dbgPrint("o");
...
//
//-----------------------------------------------------------------------------
static uint32_t blk_up_startSend_Udat(app_tuds_t *p_app_UD, uint8_t blk_No)
{
    uint32_t err_code;
    //int32_t r;
    ...
    //BlkUp_timer_stop();
    err_code = ble_tuds_notify_Udat(p_app_UD->p_ble_tuds, m_Udat_buf, &m_Udat_len);
    if(err_code == NRF_SUCCESS)
    {
        dbgPrint("o");
...
//-----------------------------------------------------------------------------
static void whack(app_tuds_t *p_app_UD)
{
    p_app_UD->m_BlkUp_sm = eBlkUp_DAT_PRESEND;
    m_current_blk_No = 0;
...
//
//-----------------------------------------------------------------------------
PUBLIC int32_t app_tuds_Ucfm_handler(app_tuds_t *p_app_UD, uint8_t *buf, uint8_t len)
{
    int32_t i;
    int32_t r;
    r = 0;
...
//
//-----------------------------------------------------------------------------
PUBLIC uint32_t app_tuds_RegCb_UpEventHandler(app_tuds_t *p_app_UD, blkUp_UpEventHandler_t cb)
{
    p_app_UD->blkUp_UpEventHandler = cb;
    return(0);
}
...
//-----------------------------------------------------------------------------
PUBLIC int32_t app_tuds_Ucmd_handler_OnWrittenComplete(app_tuds_t *p_app_UD,  uint8_t *buf, uint8_t len)
{
    int32_t r;
...
// Callback for write done event
//-----------------------------------------------------------------------------
static void doSetAnEventToFakeACallToThisAgain(app_tuds_t *p_app_UD)
{
  p_app_UD->m_BlkUp_sm = eBlkUp_DAT_SEND;
...
//-----------------------------------------------------------------------------
#define SAD_BURST 1
PUBLIC int32_t app_tuds_Udat_handler_OnWrittenComplete(app_tuds_t *p_app_UD,  uint8_t *Xbuf, uint8_t Xlen) //int32_t BlkUp_On_written_Udat(ble_tuds_t *p_tuds,  uint8_t *buf, uint8_t len)
{
    uint32_t err_code;
    int32_t r;
...
//
//-----------------------------------------------------------------------------
static void blk_up_printState(app_tuds_t *p_app_UD)
{
...
*/        
    ...
    {
        app_tuds_Udat_handler_OnWrittenComplete(p_app_UD, 0, 42); // at timeout in eBlkUp_DAT_SEND state
...
void app_tuds_BlkUp_Go_Test(app_tuds_t *p_app_UD)
{
    ...
    dbgPrintf("app_tuds_BlkUp_Go_Test\n\r");
...
//
//-----------------------------------------------------------------------------
PUBLIC int app_tuds_U_StartSendPacket(app_tuds_t *p_app_UD, uint8_t *pkt, uint16_t len)
{
    dbgPrintf("app_tuds_U_StartSendPacket\r\n");
...
//
//-----------------------------------------------------------------------------
PUBLIC uint32_t app_tuds_U_onBleConnect(app_tuds_t *p_app_UD)
{
    return(0);
}
...
//
//-----------------------------------------------------------------------------
PUBLIC uint32_t app_tuds_U_onBleDisconnect(app_tuds_t *p_app_UD)
{
    BlkUp_timer_stop();
    p_app_UD->m_BlkUp_sm = eBlkUp_IDLE;
    return(0);
