...
/**/
    ...
    for(i=0;i<SB_cnt;i++) {
        if( i>0 )
            ...
            delta = 0;
        deltaT = delta *(1000.0/32768.0);
        deltaTT += deltaT;
        dbgPrintf("  CCr = %d,  SB_blk[%d] = %d,  count = %d, delta = %d, deltaT = %f\n\r", SB_blk[i].CCr, i, SB_blk[i].cnt, SB_blk[i].count, delta, deltaT);
    }
    ...
    dbgPrintf(" d = %d, dt = %f \r\n", _d, _dt);
    dbgPrintf(" Total Time = %f, Tt  / connInt = %f \r\n", deltaTT, deltaTT/9.0);
/**/
}
...
//
//-----------------------------------------------------------------------------
PUBLIC uint32_t app_ud2_RegCb_UpEventHandler(app_ud2_t *p_app_UD, blkUp_UpEventHandler_t cb)
{
...
{
  p_app_UD->m_BlkUp_sm = eBlkUp_DAT_SEND;
  BlkUp_timer_start(p_app_UD, 500, NULL); // 500-> 
  ...
  //BlkUp_timer_start(p_app_UD,  80, NULL); // 80-> Cr every 4~6 packets
...
//
//-----------------------------------------------------------------------------
PUBLIC int app_ud2_U_StartSendPacket(app_ud2_t *p_app_UD, uint8_t *pkt, uint16_t len)
{
    dbgPrintf("app_ud2_U_StartSendPacket\r\n");
...
//
//-----------------------------------------------------------------------------
PUBLIC uint32_t app_ud2_U_onBleConnect(app_ud2_t *p_app_UD)
{
    return(0);
}
...
//
//-----------------------------------------------------------------------------
PUBLIC uint32_t app_ud2_U_onBleDisconnect(app_ud2_t *p_app_UD)
{
    BlkUp_timer_stop();
    p_app_UD->m_BlkUp_sm = eBlkUp_IDLE;
    return(0);
