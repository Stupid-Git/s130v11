...
{   int pos = 0;
    ...
    *pw = 0;
    ...
    while(1)
    {
        while( app_uart_get((uint8_t*)p_byte) != NRF_SUCCESS) ;

        if( *p_byte == 0x08)
        {   if( pos > 0 )
            {
                while( app_uart_put((uint8_t)(*p_byte)) != NRF_SUCCESS) ; //echo         
                while( app_uart_put(' ') != NRF_SUCCESS) ; //echo         
                while( app_uart_put((uint8_t)(*p_byte)) != NRF_SUCCESS) ; //echo
                pos--;
                pw--;
            }
...
int  k_scanf(const char* format, void* value)
{
    int r;
    const char line[128];
    char* lp;
...
*/
void print_parameters(radio_tests_t test)
{
    printf("Parameters:\r\n");
    switch(mode_)
    {
    ...
    switch(test)
    {
        case RADIO_TEST_NOP:      /**< No test running.      */
            printf("Test ...............: None\r\n");
            break;
        case RADIO_TEST_TXCC:     /**< TX constant carrier.  */
            printf("Test ...............: Tx Carrier\r\n");
            break;
        case RADIO_TEST_TXMC:     /**< TX modulated carrier. */
            printf("Test ...............: Tx Modulated\r\n");
            break;
        case RADIO_TEST_TXSWEEP:  /**< TX sweep.             */
            printf("Test ...............: Tx Sweep\r\n");
            break;
        case RADIO_TEST_RXC:      /**< RX constant carrier.  */
            printf("Test ...............: Rx Carrier\r\n");
            break;
        case RADIO_TEST_RXSWEEP:  /**< RX sweep.             */
            printf("Test ...............: Rx Sweep\r\n");
            break;
    }
...
{ 
    ...
    const app_uart_comm_params_t comm_params =
    {
        ...
        RTS_PIN_NUMBER,
        CTS_PIN_NUMBER,
        //APP_UART_FLOW_CONTROL_ENABLED, //karel
        APP_UART_FLOW_CONTROL_DISABLED, //karel
        false,
    ...
    while (true)
    {
        ...
        printf("\r\n");
        printf("Command (h): ");
        control = 'h'; // if scan fails, revert to 'h';
        r = k_scanf("%c",&control);
        //if( r==0)
        //{
        //    control = 'h';
